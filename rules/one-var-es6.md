# Require or Disallow One Variable Declaration per Scope (one-var-es6)
...
This rule is aimed at enforcing the use of either one variable declaration or multiple declarations per function. As such, it will warn when it encounters an unexpected number of variable declarations. It ignores es6 declarations from `let` and `const`.
